...
class ClaudeProvider implements AIProviderInterface {
    ...
    public function analyzeTicket(string $title, string $content): ?array {
        $api_key = PluginConfig::getConfig('claude_api_key', '');
...
Description: {$content}";
        ...
        $response = $this->callApi($prompt, (int)PluginConfig::getConfig('claude_max_tokens', 1024));
...
Génère 4 à 5 pistes de résolution concrètes et adaptées à ce ticket. Soit bref. Réponse en HTML (<ul><li>...</li></ul>). Aucune intro ou outro.";
        return $this->callApi($prompt, (int)PluginConfig::getConfig('claude_max_tokens', 1024));
    }

    private function callApi(string $prompt, int $maxTokens): ?string {
        $api_key = trim(PluginConfig::getConfig('claude_api_key', ''));
        $model = trim(PluginConfig::getConfig('claude_model', 'claude-3-haiku-20240307'));
        $timeout = (int)PluginConfig::getConfig('claude_timeout', 15);
        $temperature = (float)PluginConfig::getConfig('claude_temperature', 0.4);
